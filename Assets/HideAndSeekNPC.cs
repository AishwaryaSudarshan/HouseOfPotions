...
public class HideAndSeekNPC : Agent
{
    ...
    {
        // Reset NPC position (e.g., to a starting point)
        // transform.localPosition = new Vector3(0, 0, 0); // Example reset position
        knowsPlayerLocation = false;
        navMeshAgent.speed = patrolSpeed;
        // Optionally reset player position too for consistent training start
    ...
    {
        if (patrolPoints.Length == 0) return; // No points defined
