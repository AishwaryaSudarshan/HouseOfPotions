...
public class FenceLayoutEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUIStyle italic = new(GUI.skin.label)
        {
        ...
        if (ShowPoints)
        {
            ...
            if (GUILayout.Button("Add point"))
            {
                ...
                {
                    Vector3 dir = m_fenceLayout.FencePoints[^1] - m_fenceLayout.FencePoints[^2];
                    point = m_fenceLayout.FencePoints[^1] + (dir.normalized * 3.0f);
                }
                else if (m_fenceLayout.FencePoints.Count > 0)
                {
                    point = m_fenceLayout.FencePoints[^1];
                    point.x += 3.0f;
                }
                else
                {
    ...
    private void InputUpdate(SceneView sceneview)
    {
        ...
        if (e.type is not EventType.Repaint and not EventType.Layout)
        {
            ...
            if (fenceObj)
            {
                ...
                if (fence)
                {
                    ...
                    if (e.control)
                    {
                        ...
                        if (Physics.Raycast(r, out RaycastHit result))
                        {
    ...
    public void CreateFences()
    {
                ...
                if (gotLast)
                {
                    ...
                    if (m_fenceLayout.ObjectMode)
                    {
                        ...
                        for (int i = 0; i < fenceCount; i++)
                        {
                            ...
                            Vector3 offset = new(Random.Range(-m_fenceLayout.PositionVariation.x, m_fenceLayout.PositionVariation.x), 0.0f, Random.Range(-m_fenceLayout.PositionVariation.y, m_fenceLayout.PositionVariation.y));
                        ...
                        for (int i = 0; i < numFencesInt; i++)
                        {
                            ...
                            if (m_fenceLayout.UseShear)
                            {
                                ...
                                if (Mathf.Abs(shearAngleRad) > 0.01f)
                                {
                                    ...
                                    GameObject t1 = new("Shear1");
                                    GameObject t2 = new("Shear2");
        ...
        if (!m_fenceLayout.CompleteLoop && !m_fenceLayout.ObjectMode && m_fenceLayout.PostPrefab)
        {
            if (m_fenceLayout.FencePoints.Count > 0)
            {
                ...
                if (post)
                {
                    post.localPosition = m_fenceLayout.FencePoints[^1];
        ...
        if (m_fenceLayout.UseRaycast)
        {
            ...
            if (rayLength > 0)
            {
                ...
                do
                {
                    ...
                    if (Physics.Raycast(rayStart, Vector3.down, out RaycastHit hit, rayLength, m_fenceLayout.LayerMask))
                    {
